...
const Result = () => {
  ...
  // use to keep track on loading... text
  const [isImageLoaded, setIsImageLoaded] = useState(true)
  // its used ofr tracking when to show btns whoch and what time
  const [image, setImage] = useState(assets.sample_img_1)

  const { generateImage } = StateStore()
    ...
    >
      ...
      <div>
        <div className='relative'>
          ...
          {/* if classes depend on var or something use {} `` to get the dynamic mode */}
        </div>
      ...
      {isImageLoaded && <div className='flex gap-2 flex-wrap justify-center text-white text-sm p-0.5 mt-10 rounded-full'>
        <p onClick={() => { setIsImageLoaded(false) }} className='bg-transparent border border-zinc-900 text-black px-8 py-3 rounded-full cursor-pointer'>Generate</p>
